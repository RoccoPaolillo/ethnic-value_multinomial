...
to move-turtles          ; choice for each turtle. The agent makes a patch-set combined of current location and an empty patch (the options)
  ...
  ask turtles [
   ...
   let shape-myself shape
    ...
    move-to rnd:weighted-one-of options [exp ((beta-ie * (uti-eth)) )]  ; roulette wheel: move to one option according to probability derived by expU,
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <timeLimit steps="20"/>
    <metric>ticks</metric>
    <metric>mean [count (turtles-on neighbors) with [ethnicity = [ethnicity] of myself]] of turtles with [count (turtles-on neighbors) &gt;= 1]</metric>
    ...
    <steppedValueSet variable="beta-ie" first="0" step="1" last="100"/>
