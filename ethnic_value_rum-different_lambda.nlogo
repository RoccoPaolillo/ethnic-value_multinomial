...
;    let lambda-iv ifelse-value (shape = "square") ; [lambda-iv-sq][ lambda-iv-cl]   ; lambda-iv for value composition
  ...
  ;  let i_v-myself ifelse-value (shape = "square") [i_v_sq][i_v_cl]
    ...
    move-to rnd:weighted-one-of options [exp ((lambda-ie * (uti-eth)) + (lambda-iv * (uti-val)))]  ; roulette wheel: move to one option according to probability derived by expU,
                                                                                                   ; U of each dimension = lambda_ethnic*ethnic_utility + lambda_value*value_utility
...
Same simulation and description as ethnic_value_rum. Used rnd extension for roulette-wheele: https://ccl.northwestern.edu/netlogo/docs/rnd.html.

The decision is in move-turtles block.

Probability to select a neighborhood fits discrete choice models and random utility models. Discrete choice focuses on the selection of an option out of a set of alternatives. Random utility is based on the maximization of utility depending on the characteristics of the option. The utility function and preferences (here i_e/i_v) and utility function implemented (threshold,single-peaked,linear+constant) define how agents assess neighborhood’s characteristics. Lambdas (betas in literature) are the equivalent of coefficients of regression and define how important each dimension of systematic utility is on making a choice. It is necessary to have one lambda for each dimension of utility: U: lambda_ethnic * ethnic utility + lambda_value * value utility. Peculiar to random utility is that utility is a random variable unknown to the researcher and the assumption that the choice of an option depends uniquely on the maximization of systematic utility, selecting always the best option, must be relaxed. With McFadden and Train, the assumption of random utility model is that each probability to choose one option is a linear combination of systematic utility and a random term which represents other characteristics of the option or of the chooser which are unknown to the researcher and that influence the choice: U: (lambda_ethnic * ethnic utility) + (lambda_value * value utility) + epsilon. To what extent the choice relies on the maximization of systematic utility or it is a random choice, depends on lambdas. The higher the lambdas, the more the choice will depend on the difference in utility for that dimension, the lower the lambdas, the more the choice will be random for that dimension of utility. Random utility draws from the assumption of independence and identically distribution of random terms, so that systematic utility depends only on the characteristics of interest of the options (assumed the extreme value distributions, e.g. Gumbel). For this, the probability to pick an option j out of set of choice k is expUj/SumUk. 

In this simulation, probability to relocate is implemented with the roulette wheel selection through RND extension:

move-to rnd:weighted-one-of options [exp ((lambda-ie * (uti-eth)) + (lambda-iv * (uti-val)))]

where options is the list = choice set each agent builds at local level and exp((lamda_e*U_e) + (lambda-iv *U_v)) (the systematic component of utility) the size according to which one option has weighted probability to be selected. 
...
In a very simplified way, this represent how people can share different values and belong to different categories potentially independent of attributed ethnic membership and that can serve to cross ethnic boundaries and define membership in diverse societies (Wimmer, 2009). In this study, this is applied to definition of homophily preferences within residential choice. The homophily preferences of agents for ethnic and value concentration of neighborhoods, i.e. ideal concentration (i_e, i_v) and their weights (betas) in the decision to relocate depends on their value orientation. This is how interaction term in conditioanl logits are interpreted (i.e. how betas differ between different groups e.g. different ses, different ethnic group, etc.). In this view, the ACS scenario was one of possible application (ethnicity-oriented = beta-value 0, high beta-ethnic; value-oriented = beta-ethnic = 0, high beta-value).


At each step an agent compares its current location with other potential locations and makes a choice. Agents move asynchronously: one is selected randomly, makes a choice, and then the next agents move; all agents eventually make a choice.



The same for 2 options is done by the logit model as Train 
P = 1/1 + exp(lambda-ie * (uti-eth)) + (lambda-iv * (uti-val))
for each option

and the logistic function with the decision to move to alternative
P = 1/1 + exp(-lambda-ie * (uti-eth)) + (-lambda-iv * (uti-val))
...
- num_alternative: the number of alternative location (+ current node). Goes from 1 (binary choice) to all available empty patches (multinomial), tunable.

Here the utility function for ethnic and value utility and lambda_ethnic and lambda_value depend on the value orientation of the agents, independent of their ethnic group:

- i_e_sq: desired ethnic concentration squared population
- i_v_sq: desired value concentration squared population
- lambda-ie-sq: lambda weight for ethnic utility of squared population
- lambda-iv-sq: lambda weight for value utility of squared population
- i_e_cl: desired ethnic concentration circle population
- i_v_cl: desired value concentration circle population
- lambda-ie-cl: lambda weight for ethnic utility of circle population
- lambda-iv-cl: lambda weight for value utility of circle population

Chooser for utility function:
- Threshold, utility 0 below ideal point, 1 for ideal point and above levels
- Single-peaked, here how it looks like: https://docs.google.com/document/d/1ohT817vJSuf6gCnpiea6OB6wLsMYcJrOnC7nBRzLr9M/edit?usp=sharing
- linear+const: the utility is linearly increasing until the ideal concentration, constant at 1 for the levels above

Beta_distrib? Switch to activate beta distribution
- The alpha and beta parameters independent for desired ethnic concentration and value concentration of squared and value population. In total 4 distinct distributions were envisaged.

## THINGS TO NOTICE

- Ethnic segregation
- Value segregation
- Increment of utility. Utility is both for ethnic and value composition and the sum of them. I think independent ethnic and value utility  is more useful to the modeling framework to detect when the relocation was due to maximize utility or effect of interaction of other agents/random choice. If chosen as indicator.

## NEXT STEPS

Lambdas will be available with interaction: 
- lambda-ie and lambda-iv the same for all of the population (2 lambdas)
- lambda-ie and lambda-iv depending on ethnic group (4 lambdas)
- lambda-ie and lambda-iv depending on ethnic group x value orientation (8 lambdas)

Utility function simplified to linear function for all agents, and vary the lamdas. In the current model, linear function (the more the better) is equal to single-peaked function and ideal concentration equal 1; https://docs.google.com/document/d/1ohT817vJSuf6gCnpiea6OB6wLsMYcJrOnC7nBRzLr9M/edit?usp=sharing

Beta distribution will be deleted. For the next paper seems better to have uniform distribution of desired ethnic and value distribution.
