...
sens_vlmj1 <- read.csv("sens_vlmj.csv",skip=6,sep=",")
names(sens_vlmj1) <- read.csv("lg_full.csv",sep=";")[,1]

sens_vlmj2 <- read.csv("sens_vlmj2.csv",skip=6,sep=",")
names(sens_vlmj2) <- read.csv("lg_full.csv",sep=";")[,1]

sens_vlmj <- rbind(sens_vlmj1,sens_vlmj2)
sens_vlmj$ID <- "mj"


sens_vlmn <- read.csv("sens_vlmn.csv",skip = 6, sep= ",")
names(sens_vlmn) <- read.csv("lg_full.csv",sep=";")[,1]
sens_vlmn$ID <- "mn"

sens_vl <- rbind(sens_vlmj,sens_vlmn)

write.csv(sens_vl,"sens_vl.csv",row.names = F)
...
mj <- sens_vl %>% filter(ID == "mj") %>% filter(type == "Liberals Majority" | type == "Liberals Minority") %>% ggplot(aes(x=val_lib_loc,y=Segra, color = segregation, linetype = as.factor(val_lib_min))) + geom_point() + geom_line() + 
  facet_wrap(~ type)
...
# etlb_mn$ID <- "mn_et"
# 
# etlb_bs2 <- read.csv("et_lib_fc_bothfalse2.csv",sep=",",skip=6)
# names(etlb_bs2) <- lg
# etlb_bs2$ID <- "bsl"
# 
# etlb_mj2 <- read.csv("et_lib_fc_mj2.csv",sep=",",skip=6)
# names(etlb_mj2) <- lg
# etlb_mj2$ID <- "mj_et"
# 
# etlb_mn2 <- read.csv("et_lib_fc_mn2.csv",sep=",",skip=6)
# names(etlb_mn2) <- lg
# etlb_mn2$ID <- "mn_et"
# 
# etlb <- rbind(etlb_bs,etlb_mj,etlb_mn,etlb_bs2,etlb_mj2,etlb_mn2)
# 
# write.csv(etlb,"etlb.csv",row.names=F)
