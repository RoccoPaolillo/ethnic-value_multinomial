...
f_val <- function(e_w,ew_ng,i_v,S_v,Mv){
...
$P_j=\frac{1}{1 + \exp(-\beta\times \Delta_{(U_j,U_i)})}$ 



Here is the logistic function implemented in the model for the choice between current location and alternative location and the two dimensions ethnic and value utility used:
...
$k$ times ethnic weight $ew$ and $k$ times $vw$ substitute the parameters in random  utility for how important the ethnic composition and value composition are in the logistic function. My idea is that ethnic weight and value weight range from 0 to 1, whose value is too low causing random choices, by multiplying by a constant the ratio in the distribution point stay the same.\par
...
With utility calculated as above $\beta_ee_j + \beta_vv_j$ and $exp((U_k))$ the exponential function of the sum of all set of options
...
$P_i=\frac{1}{1 + \exp((\beta_eUe_j + \beta_vUv_j) - (\beta_eUe_i + \beta_vUv_i))}$

From here, in terms of two options, the probability of an agent to relocate from current node \textit{i} to alternative \textit{j}  becomes:

$P_j=\frac{1}{1 + \exp((-\beta_e{(Ue_j-Ue_i)}) + (-\beta_v{(Uv_j-Uv_i)})}$ 

I applied it to the beta distribution of ethnic weights and value weights
...
## Single Run
```{r single-run,fig.width=4,fig.height= 3, fig.align="center", include=TRUE, echo=FALSE}
...
dist_curr <- abs(ew_ng_curr - e_w)
  dist_alt <- abs(ew_ng_alt - e_w)

conc_curr <- (eth_curr / n_curr)
conc_alt <- (eth_alt / n_alt)

uti_eth_curr <- f_eth(i_e,M,eth_curr,n_curr,S)
uti_eth_alt <- f_eth(i_e,M,eth_alt,n_alt,S)

uti_val_curr <- f_val(e_w,ew_ng_curr,i_v,S_v)
uti_val_alt <- f_val(e_w,ew_ng_alt,i_v,S_v)

diff_eth <- (uti_eth_alt - uti_eth_curr)
  diff_val <- (uti_val_alt - uti_val_curr ) 

prob_one_run <- prob(diff_eth,diff_val,e_w,v_w,k)

report_one_run <- na.omit(data.frame(param_eth,param_val,i_e,i_v,S,M,dist_curr,dist_alt,conc_curr,conc_alt,uti_eth_curr,uti_eth_alt,uti_val_curr,uti_val_alt,prob_one_run))

suppressWarnings(suppressMessages(
knitr::kable(report_one_run,align="c")
))
...
e_w <- 0.2    # parameters to manipulate
i_v <- 0.2   # Symmetric = S:1,M:0/Threshold = S:0
S_v  <- 1

fv0d2 <- f_val(e_w,0,i_v,S_v)             # value utility for each option, 
fv1d2 <- f_val(e_w,0.1,i_v,S_v)           # increasing nuMber is average ethnic weight of neighborhood j
fv2d2 <- f_val(e_w,0.2,i_v,S_v)
fv3d2 <- f_val(e_w,0.3,i_v,S_v)
fv4d2 <- f_val(e_w,0.4,i_v,S_v)
fv5d2 <- f_val(e_w,0.5,i_v,S_v)
fv6d2 <- f_val(e_w,0.6,i_v,S_v)
fv7d2 <- f_val(e_w,0.7,i_v,S_v)
fv8d2 <- f_val(e_w,0.8,i_v,S_v)
fv9d2 <- f_val(e_w,0.9,i_v,S_v)
fv10d2 <- f_val(e_w,1.0,i_v,S_v)
...
report_V <- na.omit(data.frame(S_v,des_dist,ew_agent,mean_ew_ng,Utility_V))
