...
With $\beta_x$ being the importance of ethnic composition $e_j$ of neighborhood $j$, $\beta_y$ the importance of its value composition $v_j$, and $\epsilon_j$ the random term. The random term refers to other characteristics of neighborhoods or of decision makers that can influence the utility and probability of choice, but that are totally unknown. $\beta$ regulates the systematic component of utility, and $\epsilon$ the random component of utility [@bruch2009preferences]. Random component is assumed to be independent and identically distributed (i.i.d.) among options, while the systematic component of utility depends on the formalized functional form and how it interacts with $\beta$ values. The higher is the parameter $\beta \in[0,\infty)$, the lower is the effect of random term and the more utility and probability of choice is based on the observed characteristics of interest of the option (here ethnic and value composition) and how they relate to the functional form implemented.
...
Using the logistic function applies the discrete choice for a logit model [@liao1994interpreting], where the probability is calculated for the event to select a probability or not. In terms of residential move as in the model, $P=1$ means to select the choice, $P=0$ means to not select, and consequently to stay with the current option.
...
This fits the discete choice model for logit model [@liao1994interpreting], which calculates the probability of an event to occur or not.


take on to happen outcome of a probability to occur or not. Given two options, P=1 means choosing the option, P=0 necessary to choose the other alternative. 



under the this justifies using the
one differentiation between discrete choice models is:\par
- Logit model: used for 2 options using the logitisc function. Since the probability calculated estimates the chance to choose one option with $P=1$, it can be used for 2 options where $P=0$ means 



used for two options, the probability calculated with logistic function $P_j=\frac{1}{1 + \exp(-\lambda\times \Delta_{(U_j,U_l)})}$ refers to the event that one option is chosen choose one option with $P_j=1$ 


- Conditional logit model for multinomial distribution: used for more than two options. The probability $P_j=\frac{\exp(U_{(xjyj)})}{\sum_{l}exp((U_{xl,yl}))}$ from @mcfadden1973conditional calculates how one option will be selected out of the set of discrete choices

As I summarized via e.mail, for two options a multinomial choice model can be considered substantially a binary logit model, in our case since the. The two computations of probability above both assume that the random term $\epsilon$ is idependently and identically distributed among options, and the systematic difference


the assumptions of random utility model on systematic variation in utility and independent and identical distribution of the random term 
...
fv0d2 <- f_val(e_w,0,M,i_v,S)             # value utility for each option, 
fv1d2 <- f_val(e_w,0.1,M,i_v,S)           # increasing number is average ethnic weight of neighborhood j
...
report_V <- na.omit(data.frame(M,S,des_dist,ew_agent,mean_ew_ng,Utility_V))
