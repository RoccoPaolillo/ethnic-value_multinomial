...
#f_dist <- function(o,ot,M,i,S){
#  if (abs(o-ot) < i) {
#    U <- (abs(o-ot)/i) * S
#    }else if (abs(o-ot) > i){
#      U <- M + ((1-abs(o-ot))*(1-M))/(1-i)
#    }else{
#        U <- 1}
#  round(U,digits = 3)
#  }
...
#f_val <- function(o,ot,M,i,S){
# if (round(abs(o-ot), digits=2) >= i) {
#    U <- 1
#  }else {
#      U <- # (1 - (1-(abs(o-ot))/i) * S)
#         1 -  (M + ((1-(1-abs(o-ot)))/(1-M))/(1-(1-i)))
#       1 - (M + ((1-(1-abs(o-ot)))*(1-M))/(1-(1-i)))
#    }
#  round(U,digits = 3)
#  }

#f_val_repr <- function(sim,M,i,S){
#  if (sim <= i) {
#    U <- sim/i * S
#     }else if (sim > i){
# U <- M + ((1-sim)*(1-M))/(1-i)
...
f_val <- function(o,ot,M,i,S){
  if (round(abs(o-ot), digits=2) <= i) {
    U <- 1
      ...
      U <- (M + ((1-(round(abs(o-ot), digits=2)))*(1-M))/(1-i)) * S
    }
  round(U,digits = 2)
  }
...
for (o in seq (0.0,1.0, by = 0.1)){

fv0d2 <- f_val(o,0,M,i,S)
fv1d2 <- f_val(o,0.1,M,i,S)
fv2d2 <- f_val(o,0.2,M,i,S)
fv3d2 <- f_val(o,0.3,M,i,S)
fv4d2 <- f_val(o,0.4,M,i,S)
fv5d2 <- f_val(o,0.5,M,i,S)
fv6d2 <- f_val(o,0.6,M,i,S)
fv7d2 <- f_val(o,0.7,M,i,S)
fv8d2 <- f_val(o,0.8,M,i,S)
fv9d2 <- f_val(o,0.9,M,i,S)
fv10d2 <- f_val(o,1.0,M,i,S)

value_mean_neighborhood <- seq(0.0, 1.0, by = 0.1)
Utility_V <- c(fv0d2,fv1d2,fv2d2,fv3d2,fv4d2,fv5d2,fv6d2,fv7d2,fv8d2,fv9d2,fv10d2)
name <- paste0("Ethnic weight i = ", o,";distance desired i = ",i,"; M = ",M,"; S = ",S)

plot(value_mean_neighborhood,Utility_V, xlab = "Mean ethnic weight neighborhood j", ylab = "Value Utility",  main = "Value Utility for neighborhood j of agent i",  sub = name)

report <- paste0("M = ",M," ??? S = ",S," ??? Ethnic weight i = ",o, " ??? desired distance i = ",i," ??? Mean ethnic weight j = ", seq(0.0, 1.0, by = 0.1), " ??? Utility ij = ", Utility_V)
...
```{ethnic utility, moore neighborhood}

f_eth <- function(p,M,x,n,S){
  if (x  < (n*p)){
    U <- (x / (n*p)) * S
    }else if (x > (n*p)){
      U <- M + ((1-(x/n))*(1-M))/(1-p)
...
ethnic_concentration_neighborhood <- seq(0.0, 0.8, length.out = 9)
Utility_E <- c(f0m,f1m,f2m,f3m,f4m,f5m,f6m,f7m,f8m)

# print(utilitym)
print(c("Ethnic Utility",Utility_E)) 
plot(ethnic_concentration_neighborhood,Utility_E,title("Ethnic Utility"))
...
prob <- function(l,s,t,d,p,q){1 / (1 + exp(-l*(s-t)) + exp(-d*(p-q)))}

prob1 <- prob(100,300,100,1,100,100)
plot(prob1)
```
