...
f_val <- function(e_w,ew_ng,M,i_v,S_v){
  if (round(abs(e_w-ew_ng), digits=2) <= i_v) {
    U <- 1
    }else{
      U <- (M + ((1-(round(abs(e_w-ew_ng), digits=2)))*(1-M))/(1-i_v)) * S_v
      }
  round(U,digits = 3)
  }
...
\\
\left(M + \frac{(1 - \mid{\overline{ew}_j - ew_i}\mid) \times (1 - M)}{1 - i_v}\right) \times S_v, & otherwise
\end{cases}
$$
...
fvf0p  <- f_val(e_w,0,M,i_v,S_v)
fvf1p  <- f_val(e_w,0.1,M,i_v,S_v)
fvf2p   <- f_val(e_w,0.2,M,i_v,S_v)
fvf3p   <- f_val(e_w,0.3,M,i_v,S_v)
fvf4p   <- f_val(e_w,0.4,M,i_v,S_v)
fvf5p   <- f_val(e_w,0.5,M,i_v,S_v)
fvf6p   <- f_val(e_w,0.6,M,i_v,S_v)
fvf7p   <- f_val(e_w,0.7,M,i_v,S_v)
fvf8p   <- f_val(e_w,0.8,M,i_v,S_v)
fvf9p  <- f_val(e_w,0.9,M,i_v,S_v)
fvf10p  <- f_val(e_w,1.0,M,i_v,S_v)
...
fvf0t <- f_val(e_w,0,M,i_v,S_v)
fvf1t  <- f_val(e_w,0.1,M,i_v,S_v)
fvf2t   <- f_val(e_w,0.2,M,i_v,S_v)
fvf3t   <- f_val(e_w,0.3,M,i_v,S_v)
fvf4t   <- f_val(e_w,0.4,M,i_v,S_v)
fvf5t   <- f_val(e_w,0.5,M,i_v,S_v)
fvf6t  <- f_val(e_w,0.6,M,i_v,S_v)
fvf7t   <- f_val(e_w,0.7,M,i_v,S_v)
fvf8t   <- f_val(e_w,0.8,M,i_v,S_v)
fvf9t  <- f_val(e_w,0.9,M,i_v,S_v)
fvf10t  <- f_val(e_w,1.0,M,i_v,S_v)
